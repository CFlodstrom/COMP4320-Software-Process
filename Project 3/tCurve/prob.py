...
def _integrate(t, n, _f):  
    #variables
    #epsilon adjusted to ten-thousandths place instead of thousandths for accuracy
    epsilon = 0.0001
    simpsonOld = 0.0
    simpsonNew = epsilon
    highBound = t
    ...
    while (abs((simpsonNew - simpsonOld) / simpsonNew) > epsilon):
            ...
            simpsonNew = _f(lowBound, n) + _f(highBound, n)
            
            #number of terms is s + 1
            #even terms will be multiplied by 4
            #odd terms will be multiplied by two
            for j in range (1, s + 1, 1):
                #even terms
                if (j % 2 == 0):
                    simpsonNew = simpsonNew + _f(lowBound + w *(j), n) * 4
                    
                #odd terms   
                elif (j % 2 == 1):
                    simpsonNew = simpsonNew + _f(lowBound + w *(j), n) * 2
                    
            #will check to ensure it is accurate enough to pass   
            simpsonNew = (w /  3) * simpsonNew
            
            #increases slices for next part of loop         
            s = s * 2
